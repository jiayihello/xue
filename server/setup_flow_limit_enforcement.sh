...
if [ ! -f "flow_limit_enforcer.py" ]; then
    echo "❌ 未找到 flow_limit_enforcer.py"
    exit 1
fi
...
ExecStart=/usr/bin/python3 $SCRIPT_DIR/flow_limit_enforcer.py check
...
echo "添加crontab定时任务..."
(crontab -l 2>/dev/null; echo "*/5 * * * * cd $SCRIPT_DIR && python3 flow_limit_enforcer.py check >/dev/null 2>&1") | crontab -
